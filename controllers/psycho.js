...
module.exports.createPsycho = async (req, res, next) => {
  try {
    ...
    const psycho = new Psycho({
      ...
      duration: +duration,
      TeacherId: TeacherId,
      PrivateSchoolId: req.privateSchoolId,
    });
    await psycho.save();
...
module.exports.getPsychoById = async (req, res, next) => {
  ...
  try {
    const psycho = await Psycho.findOne({
      where: { id: psychoId },
      include: { all: true },
    });
...
module.exports.getAllPsychoForTeacher = async (req, res, next) => {
  ...
  try {
    const psychos = await Psycho.findAll({
...
module.exports.getAllPsychoForStudent = async (req, res, next) => {
  ...
  try {
    const psychos = await PsychoStudent.findAll({
      where: { StudentId: studentId, status: 1 },
      include: { all: true },
    });
    res.status(200).json({ psychos });
  } catch (err) {
...
module.exports.registerPsycho = async (req, res, next) => {
  ...
  try {
    ...
    const psycho = await Psycho.findOne({ where: { id: psychoId } });
    const foundRegiseredPsycho = await PsychoStudent.findOne({
      where: { StudentId: studentId, PsychoId: psychoId },
    });
    if (foundRegiseredPsycho) {
      return res
        .status(401)
        .json({ isRegister: true, message: "تم طلب هذه الجلسة مسبقا" });
    }
    ...
    const regiseredPsycho = new PsychoStudent({
      description: description,
      StudentId: studentId,
      PsychoId: psychoId,
    });
...
module.exports.acceptPsycho = async (req, res, next) => {
  ...
  try {
    const foundRegiseredPsycho = await PsychoStudent.findOne({
      where: { id: requestPsycho },
    });
    const student = await Student.findOne({
      where: { id: foundRegiseredPsycho.StudentId },
    });
    const psycho = await Psycho.findOne({
      where: { id: foundRegiseredPsycho.PsychoId },
    });
    if (psycho.price > student.money) {
      const error = new Error("المبلغ غير كافي");
      error.statusCode = 422;
...
module.exports.getAcceptedPsycho = async (req, res, next) => {
  ...
  try {
    const psychos = await PsychoStudent.findAll({
      where: { PsychoId: psychoId, status: 1 },
      include: { all: true },
    });
    res.status(200).json({ psychos });
  } catch (err) {
...
module.exports.getrequestededPsycho = async (req, res, next) => {
  ...
  try {
    const psychos = await PsychoStudent.findAll({
      where: { PsychoId: psychoId, status: 0 },
      include: { all: true },
    });
    res.status(200).json({ psychos });
  } catch (err) {
